...
class RhNet(nn.Module):
    ...
    def sod_loss_fn(self, x, y):
        return weighted_bce_loss_with_logits(x, y, reduction='mean') + iou_loss_with_logits(x, y, reduction='mean')
    ...
    def train(self, mode=True):
        super(RhNet, self).train(mode)
        self.forward = self.forward_train
        return self

    def eval(self):
        super(RhNet, self).train(False)
        self.forward = self.forward_inference
        return self
    ...
    def forward_train(self, sample):
        ...
        if type(sample) == dict and 'gt' in sample.keys() and sample['gt'] is not None:
            ...
            loss = self.pc_loss_fn(self.des(d3, (H, W)),
                                   self.des(self.image_pyramid.reduce(d2), (H, W)).detach()) * 0.0001
            loss += self.pc_loss_fn(self.des(d2, (H, W)),
                                    self.des(self.image_pyramid.reduce(d1), (H, W)).detach()) * 0.0001
            loss += self.pc_loss_fn(self.des(d1, (H, W)),
                                    self.des(self.image_pyramid.reduce(d0), (H, W)).detach()) * 0.0001
...
# def RhNet_Res2Net50(depth, pretrained, base_size, **kwargs):
#     return RhNet(res2net50_v1b_26w_4s(pretrained=pretrained), [64, 256, 512, 1024, 2048], depth, base_size, **kwargs)


def RhNet_SwinB(depth, pretrained, base_size, **kwargs):
    return RhNet(SwinB(pretrained=pretrained), [128, 128, 256, 512, 1024], depth, base_size, **kwargs)

...
class SICA(nn.Module):
    def __init__(self, in_channel, out_channel=1, depth=64, base_size=None, stage=None, lmap_in=False):
        ...
        self.conv_query = nn.Sequential(FusionConv2d(in_channel, depth, 3, relu=True),
                                        FusionConv2d(depth, depth, 3, relu=True))
        self.conv_key   = nn.Sequential(FusionConv2d(in_channel, depth, 1, relu=True),
                                        FusionConv2d(depth, depth, 1, relu=True))
        self.conv_value = nn.Sequential(FusionConv2d(in_channel, depth, 1, relu=True),
                                        FusionConv2d(depth, depth, 1, relu=True))
        ...
        self.conv_out1 = FusionConv2d(depth, depth, 3, relu=True)
